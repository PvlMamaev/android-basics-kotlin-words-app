...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val letterId = intent
        ?.extras // предоставляет способ доступа ко всем дополнительным функциям, переданным в intent (тип bundle)
        ?.getString(LETTER) // возвращает строку с возможностью null
            .toString() // вызывается, чтобы убедиться, что это СТРОКА, а не null
